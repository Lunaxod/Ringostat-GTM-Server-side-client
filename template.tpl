...

{
  ...
  "description": "Server-side client for Ringostat platform of call tracking, telephony, end-to-end analytics.\nDon\u0027t forget to add the client.",
  "containerContexts": [
    "SERVER"
  ]
}
...
if (getRequestPath().substring(0,4) === '/rng') {
  ...
  const proxyResponse = (response, headers, statusCode) => {
    ...
    for (const key in headers) {
      if (key !== 'access-control-allow-origin') setResponseHeader(key, headers[key]);
    }
  ...
  switch (requestDir) {
    case '/v4/':
      ...
      let SCRIPT_SRC = '/v4/' + projectHash.substring(0, 2) + '/' + projectHash + '.js';
  ...
  }, {method: getRequestMethod(), timeout: 2000}, getRequestBody());
}
...
Created on 16/04/2021, 19:41:12
